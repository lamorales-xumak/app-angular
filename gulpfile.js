...
var puertoLocal = '3595';
var puertoRemoto = '3599';
var serverLocal = 'http://127.0.0.1:8080/control-hospitalario/api/v.0.1/';
var serverRemoto = 'http://app-inventario.azurewebsites.net/control-hospitalario/api/v.0.1/';
var recurso = '/control-hospitalario/api/v.0.1/';

//DESAROLLO#############################################################
//Tarea para levantar un servidor de desarrollo local
gulp.task('server-desarrollo-local', function() {
  gulp.src('app')
    .pipe(connect({
      host: hostServer,
      port: puertoLocal,
...
gulp.task('server-desarrollo-remoto', function() {
  gulp.src('app')
    .pipe(connect({
      host: hostServer,
      port: puertoRemoto,
...
//PRODUCCION############################################################
//Tarea para el servidor de Produccion
gulp.task('produccion', function() {
  gulp.src('dist')
    .pipe(connect({
      host: '127.0.0.1',
      port: '9535',
      livereloadreload: true,
      fallback: 'index.html',
      open: true
    }));
});
